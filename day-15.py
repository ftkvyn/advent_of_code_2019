...
setFieldVal(droidPos[0], droidPos[1], 1)
...
def setOutupt(val):
	global field, droidPos
	delta = directions[lastInput]
	if val == 0:
		setFieldVal(droidPos[0] + delta[0], droidPos[1] + delta[1], 2)
	elif val == 1:
		droidPos[0] += delta[0]
		droidPos[1] += delta[1]
		setFieldVal(droidPos[0], droidPos[1], 1)
		...
		droidPos[0] += delta[0]
		droidPos[1] += delta[1]
		setFieldVal(droidPos[0], droidPos[1], 3)
		printField()
		print('============= Air FOUND =============')
		print(droidPos)
		input('')
...
def printField():
	...
	for row in range(rows[0], rows[1] + 1):
		for col in range(cols[0], cols[1] + 1):
			...
			if 0 == row and 0 == col:
				field_str += 'S'
			elif droidPos[0] == row and droidPos[1] == col:
				field_str += 'D'
			else:
				if field[row][col] == 0: # unknown
					field_str += ' '
				elif field[row][col] == 1: # empty
					field_str += '.'
				elif field[row][col] == 2: # wall
					field_str += '#'
		field_str += '\n'
	clear()
	print(field_str)
