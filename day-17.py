...
def setOutupt(val):
	global field, row, col, rows, cols, robot
	if val == 10:
		row += 1
		cols[1] = max(col, cols[1])
		rows[1] = row
		col = 0
		field[row] = {}
		return
...
def findInts():
	global field, cols, rows, ints, total
	for row in range(rows[0], rows[1] + 1):
		for col in range(cols[0], cols[1] + 1):
			if field[row][col] == '.' or field[row][col] == 'X':
				continue
